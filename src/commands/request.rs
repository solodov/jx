...
fn dashboard_interactive_arg() -> Arg {
    Arg::new("interactive")
        ...
        .long_help("Continuously refresh this dashboard. Up/Down (or j/k) selects a PR; Enter opens its configured action menu. Home/End and PageUp/PageDown navigate; r refreshes; q/Esc exits. Existing terminal hyperlinks remain clickable. Configure review actions with [[repo.review_actions]] and stack status actions with [[repo.stack_status_actions]], also supported under repo.rules. These sets are independent; actions are never implicitly shared. Tab or ? in the menu previews argv, working directory, and source; repository-local actions show those details and require an explicit y confirmation. Commands run quietly with stdin closed and stdout/stderr logged to ~/.local/state/jx/jx-actions.log (XDG_STATE_HOME and JX_ACTION_LOG are supported). The pinned bottom status line shows running action names or pull-request refreshes with elapsed time, without PR identifiers or cached-reload messages. Successful actions show a brief completion notice after the list updates. Review actions with on_success = \"refresh-local\" quietly reload only local inbox state without changing the live refresh schedule. Failures retain the current rows and show a status message for ten seconds; success and cancellation notices last three seconds. Keyboard interaction clears notices without consuming the key or hiding ongoing work. Action failures show the action name followed by 'failed, see <path>', with the log's home directory shortened to ~; errors without a recorded log show their cause directly. Status messages have no details popup. The status row returns to the content when cleared. Esc closes the action menu or its command preview first, otherwise cancels a running action or exits. Ctrl-C cancels a running action or exits. No actions run automatically.")
}
