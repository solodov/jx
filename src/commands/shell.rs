...
__jx_work_delete_redraw_current_line() {
  printf '\e[5n' > /dev/tty 2>/dev/null || true
}

__jx_work_delete_fzf_completion() {
  local cur="$1" output selection name path display_name display_path row
  local IFS=$'\n'
  local name_width=0 max_name_width=64
  local picker_candidates=()
  local display_candidates=()

  while IFS=$'\t' read -r name path; do
    [[ -n "$name" ]] && picker_candidates+=("$name"$'\t'"$path")
  done < <(command jx work complete --workspaces --format picker --prefix "$cur" 2>/dev/null)
  (( ${#picker_candidates[@]} > 0 )) || return 0
  if (( ${#picker_candidates[@]} == 1 )); then
    name="${picker_candidates[0]%%$'\t'*}"
    COMPREPLY=("$name")
    return 0
  fi
  ...
  for row in "${picker_candidates[@]}"; do
    ...
    display_candidates+=("$name"$'\t'"$(printf '%-*s  %s' "$name_width" "$display_name" "$display_path")")
  done

  (( ${#display_candidates[@]} > 0 )) || return 0

  output="$(printf '%s\n' "${display_candidates[@]}" | fzf --height=~60% --layout=reverse --no-sort --delimiter=$'\t' --with-nth=2 --nth=1 --query "$cur" --header='Enter: complete workspace to delete')" || {
    __jx_work_delete_redraw_current_line
    COMPREPLY=()
    return 0
  }
...
__jx_work_delete_completion() {
  ...
  if command -v fzf >/dev/null 2>&1; then
    __jx_work_delete_fzf_completion "$cur"
    return
  fi
  ...
  done < <(command jx work complete --workspaces --prefix "$cur" 2>/dev/null)
}
