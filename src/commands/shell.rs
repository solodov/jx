...
fn bash_cli_completion_script() -> String {
    ...
    script.push_str(&bash_work_remove_cd_script());
    script
}
...
fn bash_work_remove_cd_script() -> String {
    format!(
        r#"
# Generated by jx shell init bash: move out of a removed current workspace.
jx() {{
  if [[ "$1" == "work" && "$2" == "remove" ]]; then
    local output status line cd_target=""
    output="$(command jx work remove --shell-cd-target "${{@:3}}")"
    status=$?
    if [[ -n "$output" ]]; then
