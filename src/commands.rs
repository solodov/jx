...
use crate::{
    ...
    repository::{
        ...
        read_workspace_metadata, validate_workspace_name, write_github_auth_cache,
        write_github_user_name_cache, write_stack_metadata, write_workspace_metadata, ClonePlan,
        DiffToolConfig, GitHubRemote, GitHubRepository, LayoutConfig, LocalRepositoryContext,
        PullRequestActionRecord, PullRequestHistoryRecord, PullRequestStore,
        PullRequestWithHistory, RepoCheckConfig, RepoCheckTrigger, RepoHook, RepoHookEvent,
        RepoPolicyConfig, RepoPullRequestEvent, RepoPullRequestHandler, RepoRuleConfig,
        RepoSyncConfig, RepoSyncRebaseStrategy, RepoWorkItemEvent, RepoWorkItemHandler,
        RepositoryContext, RepositoryError, RepositoryIdentity, RuntimeEnvironment, ShellConfig,
        ShellZoxideMode, StackMetadata, StackMetadataNode, StoredPullRequestIdentity,
        StoredPullRequestTimeline, TokenSource, WorkflowConfig, WorkspaceMetadata,
        WorkspaceParentMetadata, PULL_REQUEST_SNAPSHOT_SCHEMA_VERSION,
    },
};
...
pub enum CommandError {
    ...
    #[error("Pull request handler `{handler}` failed for PR #{pull_request}: {message}")]
    PullRequestHandler {
        handler: String,
        pull_request: u64,
        message: String,
