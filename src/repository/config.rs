...
pub enum RepositoryError {
    ...
    #[error(
        "Unsupported workflow config key `{key}` in `{file}`. Config supports `[layout]`, `[repo]`, `[[repo.event_handlers]]`, `[[repo.path_reviewers]]`, `[[repo.rules]]`, repo `workspace_shared_paths`, `[diff]`, `[auth.keychain] service/account`, and `[shell]`; remotes and hooks are not configurable."
    )]
